...
def _doxygen_emitter(target, source, env):    
    ...
    if len(source) > 1:
        print('scons: *** Only one doxygen config file may be specified for the doxygen Builder.')
        print('           Only the first source file is parsed by the Doxygen Builder all other')
    ...
    if config.has_option('doxy_config', 'example_path'):
        input = config.get('doxy_config', 'example_path').replace('\\', '').split()
    ...
    if example_path:
        for e in example_path:
            ...
            if not os.path.isabs(e):
                e = os.path.join(config_file_path, e)
